...
const EditModulKeuangan = () => {
  ...
  const getModulById = async () => {
    const response = await axios.get(`http://localhost:5000/modulkeuangan/${id}`);
  ...
  const updateModul = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/modulkeuangan/${id}`, formData, {
        headers: {
          'Content-type': 'multipart/form-data',
        },
      });
      navigate('/modulkeuangan');
    } catch (error) {
      console.log(error);
    }
  };
...
export default EditModulKeuangan;
