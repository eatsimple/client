...
const EditLinkKeuangan = () => {
  ...
  const getLinkById = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/linkkeuangan/${id}`);
  ...
  const updateLink = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/linkkeuangan/${id}`, { nama, link });
...
export default EditLinkKeuangan;
