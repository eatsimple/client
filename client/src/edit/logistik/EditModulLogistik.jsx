...
const EditModulLogistik = () => {
  ...
  const getModulById = async () => {
    const response = await axios.get(`http://localhost:5000/modullogistik/${id}`);
  ...
  const updateModul = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/modullogistik/${id}`, formData, {
        headers: {
          'Content-type': 'multipart/form-data',
        },
      });
      navigate('/modullogistik');
    } catch (error) {
      console.log(error);
    }
  };

  if (role !== 'manager logistik' && role !== 'owner') {
...
export default EditModulLogistik;
