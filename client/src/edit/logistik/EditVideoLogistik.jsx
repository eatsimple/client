...
const EditVideoLogistik = () => {
  ...
  const getVideoById = async () => {
    const response = await axios.get(`http://localhost:5000/videologistik/${id}`);
  ...
  const updateVideo = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/videologistik/${id}`, formData, {
        headers: {
          'Content-type': 'multipart/form-data',
        },
      });
      navigate('/videologistik');
    } catch (error) {
      console.log(error);
    }
  };
...
export default EditVideoLogistik;
