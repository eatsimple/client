...
const EditLinkLogistik = () => {
  ...
  const getLinkById = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/linklogistik/${id}`);
  ...
  const updateLink = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/linklogistik/${id}`, { nama, link });
      navigate('/linklogistik');
    } catch (error) {
      console.log(error);
    }
  };
...
export default EditLinkLogistik;
