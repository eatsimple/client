...
const EditLinkMarketing = () => {
  ...
  const getLinkById = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/linkmarketing/${id}`);
  ...
  const updateLink = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/linkmarketing/${id}`, { nama, link });
      navigate('/linkmarketing');
    } catch (error) {
      console.log(error);
    }
  };
...
export default EditLinkMarketing;
