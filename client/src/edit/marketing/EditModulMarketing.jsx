...
const EditModulMarketing = () => {
  ...
  const getModulById = async () => {
    const response = await axios.get(`http://localhost:5000/modulmarketing/${id}`);
  ...
  const updateModul = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/modulmarketing/${id}`, formData, {
        headers: {
          'Content-type': 'multipart/form-data',
        },
      });
      navigate('/modulmarketing');
    } catch (error) {
      console.log(error);
    }
  };
...
export default EditModulMarketing;
