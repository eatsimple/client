...
const MarketingVideo = () => {
  ...
  const getVideo = async () => {
    try {
      const response = await axios.get('http://localhost:5000/videomarketing');
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/videomarketing/${itemId}`);
      getVideo();
  ...
  if (role !== 'marketing' && role !== 'manager marketing' && role !== 'owner') {
  ...
  return (
    <div className=" flex flex-col space-y-5">
      <div>
        <Link to="/videomarketing/add-video" className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded">
          Tambah Video Marketing
        </Link>
      </div>
              ...
              <tr key={item.id}>
                ...
                <td className="border-b flex justify-center">
                  <div className="flex space-x-3 items-center p-2">
                    <button onClick={() => window.open(`http://localhost:5000/videomarketing/download/${item.id}`, '_blank')} className="text-green-500 hover:underline hover:text-green-700">
                      Download
                    </button>

                    <Link to={`edit-video/${item.id}`} className="text-blue-500 hover:underline hover:text-blue-800">
                      Edit
                    </Link>
                    {(role === 'manager marketing' || role === 'owner') && (
