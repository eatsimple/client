...
const MarketingModul = () => {
  ...
  const getModul = async () => {
    try {
      const response = await axios.get('http://localhost:5000/modulmarketing');
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/modulmarketing/${itemId}`);
      getModul();
  ...
  return (
    <div className=" flex flex-col space-y-5">
      <div>
        <Link to="/modulmarketing/add-modul" className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded">
          Tambah Modul Marketing
        </Link>
      </div>
              ...
              <tr key={item.id}>
                ...
                <td className="border-b flex justify-center">
                  <div className="flex space-x-3 items-center p-2">
                    <button onClick={() => window.open(`http://localhost:5000/modulmarketing/download/${item.id}`, '_blank')} className="text-green-500 hover:underline hover:text-green-700">
                      Download
                    </button>
