...
const SalesLink = () => {
  ...
  const getLink = async () => {
    try {
      const response = await axios.get('http://localhost:5000/linksales');
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/linksales/${itemId}`);
      getLink();
  ...
  return (
    <div className=" flex flex-col space-y-5">
      <div>
        <Link to="/linksales/add-link" className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded">
          Tambah Link Sales
        </Link>
      </div>
