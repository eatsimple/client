...
const KeuanganLink = () => {
  ...
  const getLink = async () => {
    try {
      const response = await axios.get('http://localhost:5000/linkkeuangan');
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/linkkeuangan/${itemId}`);
      getLink();
  ...
  return (
    <div className=" flex flex-col space-y-5">
      <div>
        <Link to="/linkkeuangan/add-link" className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded">
