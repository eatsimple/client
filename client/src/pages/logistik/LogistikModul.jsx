...
const LogistikModul = () => {
  ...
  const getModul = async () => {
    try {
      const response = await axios.get('http://localhost:5000/modullogistik');
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/modullogistik/${itemId}`);
      getModul();
  ...
  return (
    <div className=" flex flex-col space-y-5">
      <div>
        <Link to="/modullogistik/add-modul" className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded">
          Tambah Modul Logistik
        </Link>
      </div>
              ...
              <tr key={item.id}>
                ...
                <td className="border-b flex justify-center">
                  <div className="flex space-x-3 items-center p-2">
                    <button onClick={() => window.open(`http://localhost:5000/modullogistik/download/${item.id}`, '_blank')} className="text-green-500 hover:underline hover:text-green-700">
                      Download
                    </button>
